...
fn test_iterate_packed() {
    ...
    let limit = 2;
    let result = crate::packed::iterate_packed(1..limit, starting_integer.clone(), None, 1);
    assert_eq!(result.last_iteration, limit);
    assert_eq!(result.end_integer, integer!("887"));

    let limit = 501;

    let result = crate::packed::iterate_packed(1..limit, starting_integer, None, 1);
    assert_eq!(result.last_iteration, limit);
    assert_eq!(
        result.end_integer,
        integer!(
